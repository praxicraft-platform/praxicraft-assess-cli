...
func Register(parent *cobra.Command, rt *runtime.Runtime) {
	...
	cmd.AddCommand(&cobra.Command{Use: "list", Short: "List integrations", RunE: func(c *cobra.Command, args []string) error {
		return cmdutil.Run(rt, func() (any, error) { return rt.API.IntegrationsList(rt.Context()) })
	}})
	cmd.AddCommand(&cobra.Command{Use: "connect-url [provider]", Args: cobra.ExactArgs(1), Short: "Get dashboard connect URL", RunE: func(c *cobra.Command, args []string) error {
		return cmdutil.Run(rt, func() (any, error) { return rt.API.IntegrationsConnectURL(rt.Context(), args[0]) })
	}})
	cmd.AddCommand(&cobra.Command{Use: "test [provider]", Args: cobra.ExactArgs(1), Short: "Test provider connection", RunE: func(c *cobra.Command, args []string) error {
