...
func Register(parent *cobra.Command, rt *runtime.Runtime) {
	...
	cmd.AddCommand(&cobra.Command{Use: "get [slug]", Args: cobra.ExactArgs(1), Short: "Get assessment", RunE: func(c *cobra.Command, args []string) error {
	...
	cases.AddCommand(&cobra.Command{Use: "list [slug]", Args: cobra.ExactArgs(1), RunE: func(c *cobra.Command, args []string) error {
		return cmdutil.Run(rt, func() (any, error) { return rt.API.AssessmentsCasesList(rt.Context(), args[0]) })
	}})
	for _, op := range []struct{ use, method string }{
		{"attach [slug]", "attach"}, {"replace [slug]", "replace"}, {"remove [slug]", "remove"},
	} {
		op := op
		var body, file string
		cc := &cobra.Command{Use: op.use, Args: cobra.ExactArgs(1), RunE: func(c *cobra.Command, args []string) error {
	...
	res.Flags().StringArrayVar(&resQ, "query", nil, "query key=value")
	cmd.AddCommand(res)
